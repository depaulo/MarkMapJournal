// @ts-nocheck
// Temporary: Disabling strict type checking for this file
// This will be refactored after the program is functional
...
async function openFromRecent(item) {
    if (!item?.handle)
        throw new Error('Recent item has no handle');
    const ok = await ensureReadPermission(item.handle);
    if (!ok)
        throw new Error('Permission denied to read recent file');
...
document.addEventListener('mousedown', (e) => {
    if (!headersMenu || headersMenu.style.display !== 'flex')
        return;
    if (headersMenu.contains(e.target))
        return;
    if (btnHeaders.contains(e.target))
        return;
...
function __hideImageMenu() {
    if (__imageMenu)
        __imageMenu.style.display = 'none';
}
function __positionImageMenu() {
    try {
        if (!__btnAddImage || !__imageMenu)
            return;
...
function __showImageMenu() {
    ...
    __imageMenu.appendChild(makeMenuItem('Online (URL)…', () => {
        __hideImageMenu();
        __addImageOnline();
    }, { icon: '🌐' }));
    __imageMenu.appendChild(makeMenuItem('Local (relative)…', () => {
        __hideImageMenu();
        __addImageLocalPick();
    }, { icon: '🖼️' }));
...
document.addEventListener('mousedown', (e) => {
    if (!__imageMenu || __imageMenu.style.display !== 'flex')
        return;
    if (__imageMenu.contains(e.target))
        return;
...
function showExportMenu() {
    try {
        ...
        exportMenu.appendChild(makeMenuItem('Markdown (Pandoc .md)', () => {
            hideExportMenu();
            exportMarkdownDownload();
        }, { icon: '📝' }));
        exportMenu.appendChild(makeMenuItem('Mindmap SVG (.svg)', () => {
            hideExportMenu();
            exportMindmapSvg();
        }, { icon: '🖼️' }));
        exportMenu.appendChild(makeMenuItem('HTML Preview (.html)', () => {
            hideExportMenu();
            exportHtmlPreview();
        }, { icon: '🌐' }));
...
function __tplHide() {
    if (__tplMenu)
        __tplMenu.style.display = 'none';
}
function __tplPandocHide() {
    if (__tplPandocMenu)
        __tplPandocMenu.style.display = 'none';
}
function __tplPos() {
    try {
        if (!__tplBtn || !__tplMenu)
            return;
...
function __tplPandocPos() {
    try {
        if (!__tplPandocBtn || !__tplPandocMenu)
            return;
...
function __tplShow() {
    ...
    for (const t of __tplOrg()) {
        orgWrap.appendChild(makeMenuItem(t.name, () => {
            __tplHide();
            __tplInsert(t.body);
        }, { icon: '🏛️' }));
    }
    ...
    if (mine.length) {
        for (const t of mine) {
            myWrap.appendChild(makeMenuItem('🧩 ' + t.name, () => {
                __tplHide();
                __tplInsert(t.body);
            }, { icon: '👤' }));
        }
    ...
    __tplMenu.appendChild(makeMenuItem('➕ New My template…', () => {
        __tplHide();
        __tplNewMy();
    }, { icon: '➕' }));
    __tplMenu.appendChild(makeMenuItem('📋 Duplicate Org → My…', () => {
        __tplHide();
        __tplDupOrgPicker();
    }, { icon: '📋' }));
    __tplMenu.appendChild(makeMenuItem('✏️ Edit My template…', () => {
        __tplHide();
        __tplEditMy();
    }, { icon: '✏️' }));
    __tplMenu.appendChild(makeMenuItem('📝 Rename My template…', () => {
        __tplHide();
        __tplRenameMy();
    }, { icon: '📝' }));
    __tplMenu.appendChild(makeMenuItem('🗑️ Delete My template…', () => {
        __tplHide();
        __tplDeleteMy();
    }, { icon: '🗑️' }));
    __tplMenu.appendChild(makeMenuSep());
    __tplMenu.appendChild(makeMenuItem('⬇️ Export My templates (JSON)…', () => {
        __tplHide();
        __tplExportMy();
    }, { icon: '⬇️' }));
    __tplMenu.appendChild(makeMenuItem('⬆️ Import My templates (JSON)…', () => {
        __tplHide();
        __tplImportMy();
    }, { icon: '⬆️' }));
...
function __tplPandocShow() {
    ...
    for (const t of __TPL_PANDOC_ORG) {
        pandocWrap.appendChild(makeMenuItem(t.name, () => {
            __tplPandocHide();
            __tplInsert(t.body);
        }, { icon: '📊' }));
    }
    ...
    __tplPandocMenu.appendChild(makeMenuItem('Export Pandoc Markdown (.md)', () => {
        __tplPandocHide();
        exportMarkdownDownload();
    }, { icon: '⬇️' }));
    __tplPandocMenu.appendChild(makeMenuSep());
...
document.addEventListener('mousedown', (e) => {
    ...
    if (!markmapOpen && !pandocOpen)
        return;
    if (__tplMenu && __tplMenu.contains(e.target))
        return;
    if (__tplPandocMenu && __tplPandocMenu.contains(e.target))
        return;
    if (__tplBtn && (e.target === __tplBtn || __tplBtn.contains(e.target)))
        return;
...
window.addEventListener('resize', () => {
    if (__tplMenu && __tplMenu.style.display === 'flex')
        __tplPos();
    if (__tplPandocMenu && __tplPandocMenu.style.display === 'flex')
        __tplPandocPos();
});
async function showRecentMenu() {
    ...
    recentMenu.appendChild(makeMenuItem('New document…', () => {
        hideRecentMenu();
        newDocument();
    }, { icon: '🆕' }));
    recentMenu.appendChild(makeMenuSep());
    recentMenu.appendChild(makeMenuItem('Browse…', () => {
        hideRecentMenu();
        openSmart();
    }, { icon: '📂' }));
    recentMenu.appendChild(makeMenuItem('Clear draft for current file', () => {
        hideRecentMenu();
    ...
    for (const item of list) {
        ...
        listWrap.appendChild(makeMenuItem(displayName, async () => {
            hideRecentMenu();
            try {
    ...
    if (list.length) {
        recentMenu.appendChild(makeMenuSep());
        recentMenu.appendChild(makeMenuItem('Clear recent files', async () => {
...
document.addEventListener('mousedown', (e) => {
    if (!recentMenu || recentMenu.style.display !== 'block')
        return;
...
function checkAndRestoreDraft(filename) {
    try {
        ...
        const ok = confirm(`Unsaved draft found for "${filename}" from ${timeStr}.\n\n` + `Do you want to restore it?`);
        if (ok) {
            md.value = draft.text;
            if (window.__cmSetText)
                window.__cmSetText(draft.text);
            dirty = true;
        ...
        hotPrime(currentSaveHandle).then(() => {
            hotTimer = setInterval(async () => {
                try {
                    ...
                    if (!lm)
                        return;
                    if (lm === fileLastSeenModified)
                        return;
                    if (hotWarnedModified === lm)
                        return;
...
function saveMapStyleState() {
    try {
        localStorage.setItem(MAP_STYLE_STORAGE_KEY, JSON.stringify({
            ...mapStyleState,
            updatedAt: Date.now(),
        }));
    }
...
function applyViewState(state, reason = 'applyViewState') {
    try {
        ...
        log(`${reason}: restored zoom/pan k=${safeState.k.toFixed(3)} x=${Math.round(safeState.x)} y=${Math.round(safeState.y)}`);
    }
    catch (e) {
...
function scheduleViewStateSave(reason = 'interaction') {
    ...
    viewSaveTimer = setTimeout(() => {
        ...
        if (s)
            saveViewState(s, `viewState ${reason}`);
    }, 300);
}
...
function markmapReady() {
    ...
    if (!ok)
        log('render(): ⚠️ markmap engine not ready yet (window.markmap missing parts)');
    return ok;
}
...
async function setAllNodesFolded(foldValue) {
    try {
        ...
        if (prevView)
            applyViewState(prevView, 'set all nodes folded: restore view');
...
async function updateMindmap(source) {
    ...
    if (!mm) {
        ...
        if (prevView)
            applyViewState(prevView, `${source}: restore after setData`);
        if (preserveFolds) {
...
function attachEditorScrollSync() {
    ...
    if (cmScroller) {
        if (_scrollSyncEl === cmScroller)
            return; // already attached
        if (_scrollSyncEl)
            _scrollSyncEl.removeEventListener('scroll', _onEditorScroll);
...
function _getApproxEditorLineFromScroll() {
    try {
        if (typeof window.__cmGetTopLine === 'function')
            return window.__cmGetTopLine();
    }
    catch { }
    const ed = _getEditorScrollEl();
    const max = ed.scrollHeight - ed.clientHeight;
    if (max <= 0 || __mdTotalLines <= 1)
        return 0;
...
function _getCurrentEditorLine() {
    try {
        if (typeof window.__cmGetCursorLine === 'function')
            return window.__cmGetCursorLine();
    }
    catch { }
    try {
...
function _scoreFuzzy(labelNorm, headingNorm) {
    if (!labelNorm || !headingNorm)
        return 0;
    if (labelNorm === headingNorm)
        return 100;
    if (headingNorm.includes(labelNorm) || labelNorm.includes(headingNorm))
        return 85;
...
function _getClickedMarkmapNodeInfo(ev) {
    ...
    try {
        if (window.d3?.select)
            datum = window.d3.select(nodeEl).datum();
    }
    catch { }
...
function jumpEditorToHeadingFromNodeText(nodeText, nodeInfo = null) {
    ...
    if (nodeInfo?.pathKey) {
        const pathMatches = headings.filter((h) => {
            if (!h.pathKey)
                return false;
            return (h.pathKey === nodeInfo.pathKey ||
                h.pathKey.endsWith(' > ' + nodeInfo.pathKey) ||
                nodeInfo.pathKey.endsWith(' > ' + h.pathKey));
        });
        if (pathMatches.length) {
            const chosen = pathMatches.length === 1 ? pathMatches[0] : _pickClosestByLine(pathMatches, curLine);
...
function attachMarkmapClickJump() {
    ...
    mapSvg.addEventListener('click', (ev) => {
        ...
        if (!ok) {
            log(`MarkmapJump: no heading match for "${t}"` +
...
function toggleEditor() {
    try {
        ...
        if (lastEditorWidth)
            editorEl.style.width = lastEditorWidth;
...
function inferCurrentExpandLevel(node) {
    ...
    walkTree(node, (n, depth) => {
        ...
        if (hasChildren && typeof f === 'number' && f > 0) {
            if (depth < minFoldDepth)
                minFoldDepth = depth;
        }
    });
    if (minFoldDepth === Infinity)
        return maxDepth + 1;
...
async function changeGlobalLevel(delta) {
    try {
        ...
        await mm.setData(data, { initialExpandLevel: 999 });
        if (prevView)
            applyViewState(prevView, 'level change: restore view');
        foldMap = captureFoldMapFromMm();
...
function renderMapStyleModalChoices() {
    ...
    if (!themeHost || !layoutHost || !nodeLengthHost || !linkHost) {
        try {
            log('❌ Map style modal render failed: missing host(s) ' +
                JSON.stringify({
                    themeHost: !!themeHost,
    ...
    Object.entries(MAP_THEME_PRESETS).forEach(([id, preset]) => {
        themeHost.appendChild(makeStyleChoice({
            group: 'theme',
            id,
            label: preset.label,
            sub: preset.sub,
    ...
    Object.entries(MAP_LAYOUT_PRESETS).forEach(([id, preset]) => {
        layoutHost.appendChild(makeStyleChoice({
            group: 'layout',
            id,
            label: preset.label,
    ...
    Object.entries(NODE_LENGTH_PRESETS).forEach(([id, preset]) => {
        nodeLengthHost.appendChild(makeStyleChoice({
            group: 'nodeLength',
            id,
            label: preset.label,
    ...
    Object.entries(MAP_LINK_PRESETS).forEach(([id, preset]) => {
        linkHost.appendChild(makeStyleChoice({
            group: 'linkStyle',
            id,
            label: preset.label,
            sub: preset.sub,
...
makeResizable(document.getElementById('splitEditor'), document.getElementById('editor'), document.getElementById('layout'), () => document.getElementById('layout').getBoundingClientRect().width - 300, () => 200);
makeResizable(document.getElementById('splitHtml'), document.getElementById('mapPane'), document.getElementById('viewer'), () => {
...
async function saveToHandle(handle, text) {
    ...
    if (typeof hotAfterSave === 'function')
        await hotAfterSave();
...
async function saveSmart() {
    ...
    if (currentSaveHandle) {
        try {
            ...
            if (!(await confirmOverwriteExternal()))
                return;
...
document.getElementById('btnSave').addEventListener('click', () => saveSmart().catch((e) => {
    if (e && e.name === 'AbortError') {
...
(function wireEditorOverlayToolsPanel() {
    function initEditorOverlayToolsPanel() {
        ...
        function runHeading(level) {
            try {
                const fn = window.__qiToggleHeading ||
...
function __copyComputedStylesToSvgClone(sourceRoot, cloneRoot) {
    try {
        ...
        function walk(src, dst) {
            if (!src || !dst || src.nodeType !== 1 || dst.nodeType !== 1)
                return;
...
function transformThreeCols(content) {
    ...
    for (const rawLine of lines) {
        ...
        if (line.startsWith('## ')) {
            ...
            else if (section.includes('column 2') ||
                section.includes('col 2') ||
                section.includes('[2]') ||
...
function __qiIndentTextarea(delta) {
    ...
    const nextLines = lines.map((ln) => {
        ...
        if (ln.startsWith('	'))
            return ln.slice(1);
        let cut = 0;
        while (cut < n && cut < ln.length && ln[cut] === ' ')
            cut++;
        return ln.slice(cut);
    });
    md.value = v.slice(0, from) + nextLines.join('\n') + v.slice(to);
...
import { EditorView, keymap, lineNumbers, highlightActiveLine, highlightActiveLineGutter, drawSelection, } from 'codemirror/view/dist/index.js';
...
import { syntaxHighlighting, defaultHighlightStyle, indentOnInput, foldGutter, foldKeymap, bracketMatching, } from 'codemirror/language/dist/index.js';
import { closeBrackets } from 'codemirror/autocomplete/dist/index.js';
...
try {
    if (!md || !host)
        throw new Error('Missing #md textarea or #cmHost');
            ...
            EditorView.updateListener.of((update) => {
                try {
                    if (update.selectionSet)
                        __cmLastSelection = update.state.selection.main;
                }
                catch { }
    ...
    window.__cmIndentLines = (delta) => {
        try {
            ...
            const pad = ' '.repeat(n);
            const sel = __cmLastSelection && typeof __cmLastSelection.from === 'number'
                ? __cmLastSelection
            ...
            for (let ln = b; ln >= a; ln--) {
                ...
                if (d > 0) {
                    ...
                    if (t.startsWith('\t'))
                        ...
                        let cut = 0;
                        while (cut < n && cut < t.length && t[cut] === ' ')
                            cut++;
                        insert = t.slice(cut);
                    }
                }
    ...
    window.__cmGetTopLine = () => {
        try {
            ...
            if (pos == null)
                return window.__cmGetCursorLine ? window.__cmGetCursorLine() : 0;
