import { renderHtmlWithShiki } from './render/html-preview.js';
import { __TPL_ORG, __TPL_PANDOC_ORG } from './data/templates-data.js';
...
function pwaDebugLog(msg, data = null) {
  try {
    ...
    try { console.warn('[PWA debug log failed]', e); } catch {}
  }
}
...
function __addImageLog(msg) { try { log(msg); } catch {} }
...
function __addImageOnline() {
  const url = prompt('Cole o endereço (URL) direto da imagem (terminando em .png/.jpg/.jpeg/.gif/.webp/.svg/.avif):');
  if (!url) { __addImageLog('AddImage: online canceled'); return; }
...
function __addImageLocalPick() {
  if (!__imageFileInput) { __addImageLog('AddImage: imageFile input missing'); return; }
...
if (__imageFileInput && !__imageFileInput.__addImageBound) {
  __imageFileInput.addEventListener('change', (e) => {
    try {
      ...
      if (!f) { __addImageLog('AddImage: local empty'); return; }
...
function __showImageMenu() {
  if (!__imageMenu) { __addImageLog('AddImage: imageMenu missing'); return; }
  __positionImageMenu();
  __imageMenu.innerHTML = '';
  __imageMenu.appendChild(makeMenuHeader('Add image'));
  __imageMenu.appendChild(makeMenuItem('Online (URL)…', () => { __hideImageMenu(); __addImageOnline(); }, { icon: '🌐' }));
  __imageMenu.appendChild(makeMenuItem('Local (relative)…', () => { __hideImageMenu(); __addImageLocalPick(); }, { icon: '🖼️' }));
...
__addImageLog('AddImage: boot check btnAddImage=' + (!!__btnAddImage) + ' imageMenu=' + (!!__imageMenu));
...
function showExportMenu() {
  try {
    ...
    exportMenu.appendChild(
      makeMenuItem('Markdown (Pandoc .md)', () => {
        hideExportMenu();
    ...
    exportMenu.appendChild(
      makeMenuItem('HTML Preview (.html)', () => {
        hideExportMenu();
        exportHtmlPreview();
      }, { icon: '🌐' })
    );
    ...
    try { log('❌ showExportMenu failed: ' + msg); } catch {}
...
// __TPL_ORG moved to ./data/templates-data.js
...
function __tplSaveMy(items){
  try {
    localStorage.setItem(__TPL_MY_KEY, JSON.stringify({ version: 1, updatedAt: Date.now(), items }));
    return true;
  } catch { return false; }
}
...
function __tplHide(){ if (__tplMenu) __tplMenu.style.display = 'none'; }
...
function __tplPickFromList(title, items, labelFn){
  if (!items.length) { showToast('No items available', 'error', 2400); return null; }
...
function __tplOrg(){ return __TPL_ORG; }
...
function __tplDupOrgToMy(org){
  ...
  if (!name) return;
  const items = __tplLoadMy();
  items.unshift({ id: __tplId(`my_${org.id}`), name: name.trim(), body: org.body, createdAt: Date.now(), updatedAt: Date.now(), fromOrgId: org.id });
  __tplSaveMy(items);
...
async function __tplImportMy(){
  try {
    ...
    if (!imported.length) { showToast('Import: no templates found', 'error', 2600); return; }
...
function __tplShow(){
  ...
  for (const t of __tplOrg()) {
    orgWrap.appendChild(makeMenuItem(t.name, () => { __tplHide(); __tplInsert(t.body); }, { icon: '🏛️' }));
  }
  ...
  if (mine.length) {
    for (const t of mine) {
      myWrap.appendChild(makeMenuItem('🧩 ' + t.name, () => { __tplHide(); __tplInsert(t.body); }, { icon: '👤' }));
    }
  } else {
  ...
  __tplMenu.appendChild(makeMenuItem('➕ New My template…', () => { __tplHide(); __tplNewMy(); }, { icon: '➕' }));
  __tplMenu.appendChild(makeMenuItem('📋 Duplicate Org → My…', () => { __tplHide(); __tplDupOrgPicker(); }, { icon: '📋' }));
  __tplMenu.appendChild(makeMenuItem('✏️ Edit My template…', () => { __tplHide(); __tplEditMy(); }, { icon: '✏️' }));
  __tplMenu.appendChild(makeMenuItem('📝 Rename My template…', () => { __tplHide(); __tplRenameMy(); }, { icon: '📝' }));
  __tplMenu.appendChild(makeMenuItem('🗑️ Delete My template…', () => { __tplHide(); __tplDeleteMy(); }, { icon: '🗑️' }));
  __tplMenu.appendChild(makeMenuSep());
  __tplMenu.appendChild(makeMenuItem('⬇️ Export My templates (JSON)…', () => { __tplHide(); __tplExportMy(); }, { icon: '⬇️' }));
  __tplMenu.appendChild(makeMenuItem('⬆️ Import My templates (JSON)…', () => { __tplHide(); __tplImportMy(); }, { icon: '⬆️' }));

  __tplMenu.style.display = 'flex';
}

// __TPL_PANDOC_ORG moved to ./data/templates-data.js
...
__tplPandocMenu.appendChild(makeMenuSep());
  ...
  __tplPandocMenu.appendChild(
    makeMenuItem('Export Pandoc Markdown (.md)', () => {
      __tplPandocHide();
      exportMarkdownDownload();
    }, { icon: '⬇️' })
  );
  ...
  __tplPandocMenu.appendChild(makeMenuHeader('pandoc file.md -o output.pptx --reference-doc=PandocTemplateV4.pptx'));
...
async function showRecentMenu(){
  ...
  recentMenu.innerHTML = '';
	...
	recentMenu.appendChild(
	  makeMenuItem('Clear draft for current file', () => {
		hideRecentMenu();
  ...
  for (const item of list) {
    ...
    listWrap.appendChild(
      makeMenuItem(displayName, async () => {
        hideRecentMenu();
        try {
  ...
  if (list.length) {
    recentMenu.appendChild(makeMenuSep());
    recentMenu.appendChild(
      makeMenuItem('Clear recent files', async () => {
...
function hotStop(reason){
  ...
  hotWarnedModified = 0;
  hotSetStatus('');
  try { log('HotReload: stopped (' + reason + ')'); } catch {}
}
...
async function hotPrime(handle){
  try{
    ...
    try { log('HotReload: prime failed: ' + (e && e.message ? e.message : e)); } catch {}
  }
}
...
function hotStart(reason){
  try{
    if (!hotEnabledEl || !hotEnabledEl.checked){ hotStop('disabled'); return; }
    if (!currentSaveHandle){ hotStop('no handle'); return; }
    ...
    hotPrime(currentSaveHandle).then(() => {
      hotTimer = setInterval(async () => {
        try{
          if (!hotEnabledEl.checked){ hotStop('disabled'); return; }
          if (!currentSaveHandle){ hotStop('no handle'); return; }
          ...
          const ok = confirm('O arquivo "' + currentFileName + '" foi alterado externamente.\n\nRecarregar agora e perder as alterações não salvas desta aba?');
          if (ok){
            fileLastSeenModified = lm;
            hotWarnedModified = 0;
    ...
    try{ log('HotReload: start failed: ' + (e && e.message ? e.message : e)); }catch{}
  }
}
...
async function hotAfterSave(){
  try{
    ...
    externalStale = false;
    externalStaleModified = 0;
    if (hotEnabledEl && hotEnabledEl.checked) hotSetStatus('Hot ✓ ' + new Date(fileLastSeenModified).toLocaleString());
    setStatus(modeLabel());
    updateDocumentTitle();
  }catch{}
}
...
if (hotEnabledEl && !hotEnabledEl.__bound){
  hotEnabledEl.addEventListener('change', () => {
    if (!hotEnabledEl.checked){ hotStop('toggle off'); return; }
...
function modeLabel() {
  const base = currentSaveHandle ? `Writable ✓ ${currentFileName}` : `Read-only ✓ ${currentFileName}`;
...
function getCurrentMapThemeOptions() {
  const theme =
    MAP_THEME_PRESETS[mapStyleState.theme] ||
    MAP_THEME_PRESETS.default;
  ...
  return {
    color: Array.isArray(theme.colors) && theme.colors.length
      ? theme.colors
      : getDefaultMarkmapColorPalette(),

    colorFreezeLevel:
      typeof theme.colorFreezeLevel === 'number'
        ? theme.colorFreezeLevel
        : 2
  };
}
...
function getCurrentMapLayoutOptions() {
  const layout =
    MAP_LAYOUT_PRESETS[mapStyleState.layout] || MAP_LAYOUT_PRESETS.default;

  const nodeLength =
    NODE_LENGTH_PRESETS[mapStyleState.nodeLength] ||
    NODE_LENGTH_PRESETS.medium;
...
function applyViewState(state, reason = 'applyViewState') {
  try {
    ...
    const svgSel =
      mm?.svg && typeof mm.svg.call === 'function'
        ? mm.svg
        : window.d3.select(svgEl);
    ...
    const tr = window.d3.zoomIdentity
      .translate(safeState.x, safeState.y)
      .scale(safeState.k);
...
mapSvg.addEventListener('touchend', () => scheduleViewStateSave('touchend'), { passive: true });
...
function captureFoldMapFromMm() {
  ...
  try {
    ...
    foldDbg(`capture: total=${st.total} zero=${st.zero} fold1=${st.f1} fold2=${st.f2} other=${st.other} sample=[${s.sample.join(', ')}]`);
  } catch (e) {
...
function applyFoldMapToRoot(root, map) {
  try {
    ...
    foldDbg(`apply: BEFORE nodes=${pre.totalNodes}, folded=${pre.foldedNodes}, zero=${pre.explicitZero}, undef=${pre.undefinedFold}, incomingMap=${map.size}`);
    let applied = 0;
    ...
    foldDbg(`apply: AFTER  nodes=${post.totalNodes}, folded=${post.foldedNodes}, zero=${post.explicitZero}, undef=${post.undefinedFold}, applied=${applied}`);
  } catch (e) {
...
function __ensureMarkmapBoot(){
  ...
  const tick = () => {
    ...
    const ok = !!(window.markmap && window.markmap.Transformer && window.markmap.Markmap);
    if (ok) {
      try { render('boot ensureMarkmapBoot'); } catch {}
      return;
    }
    if (tries >= maxTries) {
      try { setStatus('⚠ markmap engine not loaded (check console / network)'); } catch {}
      try { showToast('Markmap engine not loaded — check console/network', 'error', 5000); } catch {}
      return;
    }
    setTimeout(tick, 250);
  };
  ...
  if (!mm) {
	...
	mapSvg.addEventListener('click', () => {
	  ...
	  setTimeout(() => {
		...
		foldDbg(`click->state: nodes=${c.totalNodes} folded=${c.foldedNodes} zero=${c.explicitZero} undef=${c.undefinedFold}`);
	  }, 0);
	});
    ...
    const preserveFolds = (source.startsWith('debounced') || source.startsWith('blur') || source.startsWith('hotReload'));
    ...
    if (preserveFolds) {
      ...
      if (afterData) {
        ...
        foldDbg(`post-setData: nodes=${c.totalNodes} folded=${c.foldedNodes} zero=${c.explicitZero} undef=${c.undefinedFold}`);
      } else {
...
function getHtmlHeadingForMdHeading(mdHeading) {
  ...
  return (
    sameText.find(x => x.occurrence === mdHeading.occurrence) ||
    sameText[0] ||
    null
  );
}
...
function _findMdAnchorPair(lineNo) {
  ...
  let lo = 0, hi = __mdHeadings.length - 1, idx = -1;
  while (lo <= hi) {
    const mid = (lo + hi) >> 1;
    if (__mdHeadings[mid].lineNo <= lineNo) { idx = mid; lo = mid + 1; }
    else { hi = mid - 1; }
  }
  return {
  ...
  try {
    ...
    if (__mdHeadings.length && __htmlHeadings.length) {
      ...
      if (a && htmlA) {
        ...
        htmlPane.scrollTop = Math.max(
          0,
          targetTop - __HTML_SYNC_OFFSET_PX
        );
...
function _getScrollRatio(el) { const max = el.scrollHeight - el.clientHeight; return max > 0 ? (el.scrollTop / max) : 0; }
function _setScrollRatio(el, ratio) { const max = el.scrollHeight - el.clientHeight; el.scrollTop = max > 0 ? (ratio * max) : 0; }
...
function _getCurrentEditorLine() {
  try { if (typeof window.__cmGetCursorLine === 'function') return window.__cmGetCursorLine(); } catch {}
  try { const pos = md.selectionStart || 0; return md.value.slice(0, pos).split(/\n/).length - 1; } catch {}
  return 0;
}
...
function _extractHeadingPathsFromMarkdown(markdownText) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    const raw = stripHeadingPrefix(
      _cleanNodeTextForMatch((m[2] || '').trim())
    );
...
function _scrollEditorToLine(lineNo, reason = 'jump') {
  try {
    if (typeof window.__cmScrollToLine === 'function') {
      ...
      let pos = 0;
      for (let i = 0; i < Math.max(0, Math.min(lineNo, lines.length - 1)); i++) pos += lines[i].length + 1;
      md.focus();
      md.selectionStart = md.selectionEnd = pos;
...
function _getClickedMarkmapNodeEl(ev) {
  ...
  while (el && el !== mapSvg) {
    if (
      el.classList &&
      el.classList.contains('markmap-node')
    ) {
      return el;
    }
...
function _getMarkmapNodePathNorms(nodeEl) {
  ...
  try {
    ...
    while (datum) {
      const raw = _cleanNodeTextForMatch(
        _readMarkmapDatumText(datum)
      );

      const norm = _normalizeHeadingText(
        stripHeadingPrefix(raw)
      );
...
function _getClickedMarkmapNodeInfo(ev) {
  ...
  const text = _cleanNodeTextForMatch(
    div?.textContent || nodeEl.textContent || ''
  );

  const norm = _normalizeHeadingText(
    stripHeadingPrefix(text)
  );
...
function jumpEditorToHeadingFromNodeText(nodeText, nodeInfo = null) {
  const labelNorm = _normalizeHeadingText(
    stripHeadingPrefix(
      _cleanNodeTextForMatch(nodeText)
    )
  );
...
// 0) Best possible match:
    ...
    if (pathMatches.length) {
      const chosen =
        pathMatches.length === 1
          ? pathMatches[0]
          : _pickClosestByLine(pathMatches, curLine);
  ...
  if (exact.length) {
    const chosen =
      exact.length === 1
        ? exact[0]
        : _pickClosestByLine(exact, curLine);
  ...
  const chosen =
    top.length === 1
      ? top[0]
      : _pickClosestByLine(top, curLine);

  _scrollEditorToLine(
    chosen.lineNo,
    `fuzzy heading match score=${chosen.score}`
  );
  ...
  if (!mapSvg) return;
	mapSvg.addEventListener('click', (ev) => {
	  ...
	  if (nodeEl) {
		...
		mapSvg.querySelectorAll('.markmap-node.__active')
...
function suppressNextBlurRender(ms) { window.__suppressBlurRenderUntil = Date.now() + (ms || 900); }
...
function getMaxDepth(node) {
  let max = 0;
  walkTree(node, (_n, d) => { if (d > max) max = d; });
  return max;
}
...
async function changeGlobalLevel(delta) {
  try {
    ...
    if (globalExpandLevel == null) {
      ...
      log(`Level init: inferred globalExpandLevel=${globalExpandLevel} (min=${minLevel}, max=${maxLevel})`);
    }
    const next = Math.min(maxLevel, Math.max(minLevel, globalExpandLevel + delta));
    if (next === globalExpandLevel) {
      log(`Level change blocked: already at limit (level=${globalExpandLevel}, min=${minLevel}, max=${maxLevel})`);
      return;
    }
    ...
    foldDbg(`level change done: level=${globalExpandLevel} nodes=${c.totalNodes} folded=${c.foldedNodes} zero=${c.explicitZero} undef=${c.undefinedFold}`);
  } catch (e) {
...
function wireMapOverlayControls() {
  try {
    ...
    if (!overlay) {
      try { log('Map overlay controls: overlay not found'); } catch {}
      return;
    }
	...
	if (btnTheme && !btnTheme.__bound) {
	  btnTheme.addEventListener('click', async () => {
		try {
		  ...
		  const msg = e?.message || String(e);
		  try { log('❌ overlay dark mode failed: ' + msg); } catch {}
    ...
    try { log('Map overlay controls: wired'); } catch {}
  } catch (e) {
    try { log('❌ wireMapOverlayControls failed: ' + (e?.message || e)); } catch {}
  }
}
...
function applyMapStyleCss() {
  try {
    ...
    try { log('❌ applyMapStyleCss failed: ' + (e?.message || e)); } catch {}
  }
}
...
async function applyMapLayoutOptions() {
  try {
	...
	try {
	  log(`Map style options: theme=${mapStyleState.theme} color=${opts.color ? 'set' : 'none'} freeze=${opts.colorFreezeLevel ?? '(none)'}`);
	} catch {}
    
	const prevView = typeof getCurrentViewState === 'function'
      ? getCurrentViewState()
      : null;
    ...
    try { log('❌ applyMapLayoutOptions failed: ' + (e?.message || e)); } catch {}
  }
}
...
function resetMapStyleToDefault() {
  ...
  try { log('Map style: reset to default'); } catch {}
}
...
function makeStyleChoice({ group, id, label, sub, selected, swatches }) {
  ...
  btn.addEventListener('click', async () => {
    ...
    try { log(`Map style: ${group}=${id}`); } catch {}
  });
...
function wireMapStyleModifier() {
  try {
    ...
    try { log('Map style modifier: wired'); } catch {}
  } catch (e) {
    try { log('❌ wireMapStyleModifier failed: ' + (e?.message || e)); } catch {}
  }
}
...
function isTopLevel() { try { return window.self === window.top; } catch { return false; } }
function openPickerUsable() { return isTopLevel() && window.isSecureContext && ('showOpenFilePicker' in window); }
function savePickerUsable() { return isTopLevel() && window.isSecureContext && ('showSaveFilePicker' in window); }
...
async function openSmart() {
  ...
  if (openPickerUsable()) {
    ...
    log(`openSmart(): writable picker not usable. secure=${window.isSecureContext}, api=${'showOpenFilePicker' in window}, top=${isTopLevel()}`);
  }
...
function newDocument() {
  try {
    ...
    const msg = e?.message || String(e);
    try { log('❌ newDocument failed: ' + msg); } catch {}
...
fileInput.addEventListener('change', async (e) => {
  try {
    const f = e.target.files && e.target.files[0];
    if (!f) { log('read-only open: no file selected'); return; }
    currentFileName = f.name || 'markmap.md';
    updateDocumentTitle();
    currentSaveHandle = null;
    md.value = await f.text();
...
async function saveAsSmart(text) {
  ...
  if (savePickerUsable()) {
    ...
    log(`saveAsSmart(): picker not usable. secure=${window.isSecureContext}, api=${'showSaveFilePicker' in window}, top=${isTopLevel()}`);
  }
...
async function confirmOverwriteExternal() {
  try {
    ...
    try {
      const ok = confirm('Não foi possível verificar se o arquivo mudou externamente. Deseja continuar salvando?');
      return !!ok;
    } catch {
      return true;
    }
...
async function copyMarkdownToClipboard() {
  try {
    ...
    try { log('Editor overlay: markdown copied'); } catch {}
  } catch (e) {
    showToast('Copy markdown failed', 'error');
    try { log('❌ copyMarkdownToClipboard failed: ' + (e?.message || e)); } catch {}
  }
}

function exportMarkdownDownload() {
  try {
    const text = transformLayouts(md.value);
...
function wireEditorOverlayControls() {
  try {
    ...
    if (!overlay) {
      try { log('Editor overlay controls: overlay not found'); } catch {}
      return;
    }
    ...
    try { log('Editor overlay controls: wired'); } catch {}
  } catch (e) {
    try { log('❌ wireEditorOverlayControls failed: ' + (e?.message || e)); } catch {}
  }
}
...
function insertCheckboxFromOverlay(checked) {
  try {
    ...
    try { log(`❌ CodeMirror checkbox helper failed: ${e?.message || e}`); } catch {}
  }
  ...
  if (!document.body.classList.contains('cmFailed')) {
    try { log('Checkbox insert skipped: CodeMirror checkbox helper missing'); } catch {}
    try { showToast('Checkbox helper not ready', 'error', 2400); } catch {}
    return;
  }
  ...
  try {
    ...
    try { log(`❌ Checkbox textarea fallback failed: ${e?.message || e}`); } catch {}
  }
}
...
(function wireEditorOverlayToolsPanel() {
  function initEditorOverlayToolsPanel() {
    ...
    if (!overlay || !btnToggle || !panel) {
      try { log('Editor overlay tools panel: missing elements'); } catch {}
      return;
    }
...
function runEditorCommand(commandName) {
  try {
    ...
    if (!fn) {
      try { log(`Editor overlay: unknown command ${commandName}`); } catch {}
      try { showToast(`Unknown editor command: ${commandName}`, 'error', 2400); } catch {}
      return;
    }
    ...
    try { log(`❌ Editor overlay command failed (${commandName}): ${e?.message || e}`); } catch {}
    try { showToast('Editor command failed', 'error', 2400); } catch {}
  }
}
    ...
    function runHeading(level) {
      try {
        ...
        if (!fn) {
          try { log('Editor overlay: __qiToggleHeading missing'); } catch {}
          try { showToast('Heading command missing', 'error', 2400); } catch {}
          return;
        }

        fn(level);
      } catch (e) {
        try { log(`❌ Editor overlay heading failed: ${e?.message || e}`); } catch {}
        try { showToast('Heading command failed', 'error', 2400); } catch {}
      }
    }
	...
	function insertIntoEditorFromOverlay(text) {
	  ...
	  try {
		...
		try { log(`❌ Editor overlay insert failed: ${e?.message || e}`); } catch {}
	  }
	}
...
// Do not close when clicking the editor, top toolbar, map, or map overlay.
    ...
    try { log('Editor overlay tools panel: wired'); } catch {}
  }
...
function __downloadBlob(textOrBlob, filename, type = 'text/plain;charset=utf-8') {
  try {
    const blob = textOrBlob instanceof Blob
      ? textOrBlob
      : new Blob([textOrBlob], { type });
    ...
    try { log('❌ downloadBlob failed: ' + (e?.message || e)); } catch {}
    return false;
  }
}
...
function __copyComputedStylesToSvgClone(sourceRoot, cloneRoot) {
  try {
    ...
    try { log('⚠️ SVG export: computed style copy failed: ' + (e?.message || e)); } catch {}
  }
}
...
function __injectSvgExportStyle(svgEl, isDark) {
  try {
    ...
    try { log('⚠️ SVG export: style injection failed: ' + (e?.message || e)); } catch {}
  }
}
...
function exportMindmapSvg() {
  try {
    ...
    clone.setAttribute(
      'style',
      `background:${bg};display:block;width:100vw;height:100vh;`
    );
    ...
    const stamp = typeof todayStamp === 'function'
      ? todayStamp()
    ...
    try { log('❌ exportMindmapSvg failed: ' + msg); } catch {}
  }
}
...
${bodyHtml}
  ...
  <\/script>
    ...
    const base = __exportSafeBaseName(currentFileName);
    const stamp = typeof todayStamp === 'function' ? todayStamp() : new Date().toISOString().slice(0, 10);
    ...
    try { log('❌ exportHtmlPreview failed: ' + msg); } catch {}
  }
}
...
function transformLayouts(mdText) {
  ...
  for (const rawBlock of blocks) {
    ...
    if (lines[0] && lines[0].trim().toLowerCase().startsWith('layout:')) {
      layout = lines[0].replace(/^layout:/i, '').trim().toLowerCase();
      startIndex = 1;
    }
...
// ================================
function __qiSyncCm(){
  try { if (typeof window.__cmSetText === 'function') window.__cmSetText(md.value); } catch {}
}
function __qiGetSel(){
...
function __qiSetSel(start, end){
  try { md.focus(); md.selectionStart = start; md.selectionEnd = end; } catch {}
}
...
function __qiTogglePrefixTextarea(prefix){
  ...
  const nextLines = lines.map(ln => allHave ? (ln.startsWith(prefix) ? ln.slice(prefix.length) : ln) : (prefix + ln));
...
function __qiWrap(before, after, placeholder){
  if (typeof window.__cmWrapSelection === 'function') { window.__cmWrapSelection(before, after, placeholder); return; }
  __qiWrapTextarea(before, after, placeholder);
}
function __qiToggleHeading(level){
  if (typeof window.__cmToggleHeading === 'function') { window.__cmToggleHeading(level); return; }
  __qiTogglePrefixTextarea('#'.repeat(level) + ' ');
}
function __qiToggleList(prefix){
  if (typeof window.__cmToggleLinePrefix === 'function') { window.__cmToggleLinePrefix(prefix); return; }
  __qiTogglePrefixTextarea(prefix);
}
function __qiIndent(delta){
  if (typeof window.__cmIndentLines === 'function') { window.__cmIndentLines(delta); return; }
  __qiIndentTextarea(delta);
}
function __qiInsertLink(){
  if (typeof window.__cmInsertLink === 'function') { window.__cmInsertLink(); return; }
  const { start, end, text } = __qiGetSel();
...
function __qiBind(id, fn){
  ...
  el.addEventListener('pointerdown', (ev) => { try { ev.preventDefault(); } catch {} }, { passive: false });
  el.addEventListener('mousedown', (ev) => { try { ev.preventDefault(); } catch {} }, { passive: false });
  el.addEventListener('click', () => {
    try {
      if (typeof window.__cmFocus === 'function') window.__cmFocus();
      fn();
    } catch (e) { try { log('QuickInsert error: ' + (e?.message || e)); } catch {} }
  });
}
...
md.addEventListener('input', () => {
  ...
  updateDocumentTitle();
  if (renderTimer) { clearTimeout(renderTimer); renderTimer = null; log('Debounce: cleared previous render timer'); }
...
import { defaultKeymap, history, historyKeymap, indentWithTab } from "codemirror/commands/dist/index.js";
import { searchKeymap, highlightSelectionMatches, openSearchPanel } from "codemirror/search/dist/index.js";
...
import {
  closeBrackets,
  closeBracketsKeymap
} from "codemirror/autocomplete/dist/index.js";
...
function fallbackToTextarea(err) {
  ...
  try { window.log?.(`❌ CodeMirror init failed: ${err?.message || err}`); } catch {}
}
...
try {
  ...
  try { if (update.selectionSet) __cmLastSelection = update.state.selection.main; } catch {}
  ...
  try {
    ...
    try { console.warn('openSearchPanel failed:', e); } catch {}
  }
};
...
window.__cmFocus = function __cmFocus() {
 ...
 // Expose focus helper
 window.__cmFocus = () => { try { view.focus(); } catch {} };
 ...
 window.__cmIndentLines = (delta) => {
  try {
    ...
    const sel = (__cmLastSelection && typeof __cmLastSelection.from === 'number') ? __cmLastSelection : view.state.selection.main;
    ...
    try { __cmLastSelection = view.state.selection.main; } catch {}
  } catch {}
};

 // Keep last selection updated on selection changes
 try {
   view.dom.addEventListener('keyup', () => { try { __cmLastSelection = view.state.selection.main; } catch {} }, true);
   view.dom.addEventListener('mouseup', () => { try { __cmLastSelection = view.state.selection.main; } catch {} }, true);
   view.dom.addEventListener('selectionchange', () => { try { __cmLastSelection = view.state.selection.main; } catch {} }, true);
 } catch {}
...
window.__cmApplyCheckboxAtCursor = (checked) => {
  try {
    ...
    try { console.warn('__cmApplyCheckboxAtCursor failed:', e); } catch {}
  }
};

  window.__cmGetCursorLine = () => {
    try { return view.state.doc.lineAt(view.state.selection.main.head).number - 1; }
    catch { return 0; }
  };
  ...
  window.__cmWrapSelection = (before, after, placeholder='') => {
    try {
      ...
      view.dispatch({
        ...
        selection: { anchor: from + String(before).length, head: from + String(before).length + inner.length }
      });
      view.focus();
    } catch {}
  };
...
async function checkPwaServiceWorker() {
  ...
  try {
    ...
    pwaDebugLog('Service worker registrations found', regs.map(reg => ({
      scope: reg.scope,
...
async function runPwaDiagnostics() {
  ...
  await pwaFetchCheck(new URL('./manifest.webmanifest', location.href).href, 'manifest.webmanifest');
